  ...
  function executeActionInPreview(act: any) {
    switch (act.type) {
      ...
      case 'play_animation':
        if (act.target) {
          ...
          setStageElements(prev => {
            const existingIndex = prev.findIndex(el => el.type === 'video' && el.videoId === videoId);
            if (existingIndex !== -1) {
              const elVid = document.getElementById(`video_player_${prev[existingIndex].id}`) as HTMLVideoElement;
              if (elVid) elVid.play().catch(e => console.log('Video play failed:', e));
              return prev;
            } else {
      ...
      case 'stop_animation':
        if (act.target) {
          const videoId = act.target;
          const existing = stageElements.find(el => el.type === 'video' && el.videoId === videoId);
          if (existing) {
          ...
          >
            ...
            {stageElements.map(el => {
              ...
              <div
                ...
                onMouseDown={(e) => {
                  if (isPreviewing && (isButton || el.type === 'obj' || el.type === 'enemy')) {
                    e.stopPropagation();
                    e.preventDefault();
