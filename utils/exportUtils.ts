            ...
            for (const act of ev.actions) {
              if (act.type === 'play_sound' && act.value && act.value.startsWith('local_sound_ref:')) {
                ...
                try {
                  ...
                  console.error(`Failed to resolve local sound ${soundId} for compilation:`, e);
                }
              }
            }
          }
...
const playSoundWithSharedContext = async (audioSrc: string) => {
  if (!audioSrc) return;
  const normalizedSrc = normalizeDataURL(audioSrc);
  ...
  const executeAction = (act) => {
    switch (act.type) {
      ...
      case 'play_sound':
        try {
          if (act.value) {
            playSoundWithSharedContext(act.value);
          } else {
            playBeepWithSharedContext();
          }
        } catch (e) {
